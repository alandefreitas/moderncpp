...
pair<int,int> search_sequential_operations(const vector<int> &v, const int key);
...
int main() {
    ...
    // Measure real time
    cout << "n\t\tstd\t\tselection_std\t\tinsertion_std\tmerge_sort_std\tquick_sort_std" << endl;
    ...
    for (int i = 2; i < 100000000; i += increment(i+1)) {
        ...
        sort_vector(v1);
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(chrono::high_resolution_clock::now() - start);
        ...
        duration = std::chrono::duration_cast<std::chrono::microseconds>(chrono::high_resolution_clock::now() - start);
        cout << duration.count() << "μs\t\t";
        fout << duration.count() << ",";
...
void merge_sort ( Iterador start ,  Iterador fim )  {
    if ( fim -  start >  1) {
...
void quicksort(RandomAccessIterator first, RandomAccessIterator last, Order order)
{
    if (first != last && first+1 != last)
    {
        ...
        RandomAccessIterator split1 = std::partition(first, last, std::bind(order, std::placeholders::_1, pivot));
        RandomAccessIterator split2 = std::partition(split1, last, std::bind(non_strict(order), std::placeholders::_1, pivot));
