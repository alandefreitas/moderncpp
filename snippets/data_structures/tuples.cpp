...
int main() {
    ...
    std::cout << "t.first: " << t.first << ", t.second: " << t.second << std::endl;

    // Using auto
    auto t2 = std::make_pair<char,int>('c',15);
    std::cout << "t2.first: " << t2.first << ", t2.second: " << t2.second << std::endl;

    // Larger tuple
    auto t3 = std::make_tuple(6.3,  "texto" ,  2,  'o');
    int x = std::get<3>(t3);
    std::cout << "std::get<0>(t3): " << std::get<0>(t3) << ", std::get<1>(t3): " << std::get<1>(t3) << std::endl;
